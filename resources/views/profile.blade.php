...
@section('content')
<h1 style="color:#fff;font-size:24px;font-weight:700;margin-bottom:24px">Profil</h1>
...
<div class="card" style="display:flex;align-items:center;gap:20px;margin-bottom:16px">
    ...
    @else
        <div style="width:80px;height:80px;border-radius:50%;background:#1a2332;display:flex;align-items:center;justify-content:center;color:#34d399;font-size:28px;font-weight:700">{{ strtoupper(substr($user->name,0,1)) }}</div>
    @endif
    <div>
        <div style="color:#fff;font-size:20px;font-weight:700">{{ $user->in_game_name ?? $user->name }}</div>
        <div style="color:#4a5568;font-size:14px">{{ '@' . $user->username }}</div>
        @if($user->rank)
...
<div class="card" style="margin-bottom:16px">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div>
            ...
            <div style="color:#fff;font-size:14px">{{ $user->in_game_name ?? '—' }}</div>
        </div>
        <div>
            <div class="form-label">Felhasználónév</div>
            <div style="color:#fff;font-size:14px">{{ $user->username ?? '—' }}</div>
        </div>
        <div>
            <div class="form-label">Rendfokozat</div>
            <div style="color:#fff;font-size:14px">{{ $user->rank?->name ?? '—' }}</div>
        </div>
        <div>
            <div class="form-label">Utolsó előléptetés</div>
            <div style="color:#fff;font-size:14px">{{ $user->rank_up_date?->format('Y. m. d.') ?? '—' }}</div>
        </div>
        <div>
            <div class="form-label">Alosztály</div>
            <div style="color:#fff;font-size:14px">{{ $user->department?->name ?? '—' }}</div>
        </div>
        <div>
            <div class="form-label">Alosztályi rang</div>
            <div style="color:#fff;font-size:14px">{{ $user->departmentRank?->name ?? '—' }}</div>
        </div>
...
{{-- Discord account --}}
<div class="card">
    <div style="color:#fff;font-size:15px;font-weight:600;margin-bottom:14px">Discord fiók</div>
    ...
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:14px">
        ...
        @else
            <div style="width:48px;height:48px;border-radius:50%;background:#5865F2;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:18px">D</div>
        @endif
        <div>
            <div style="color:#fff;font-size:14px;font-weight:500">Discord csatolva</div>
            <div style="color:#4a5568;font-size:12px">ID: {{ $user->discord_id }}</div>
        </div>
        <span class="badge" style="background:rgba(88,101,242,.15);color:#7289da;margin-left:auto">✓ Aktív</span>
    </div>
    ...
    <a href="{{ route('auth.discord.link') }}" style="display:inline-flex;align-items:center;gap:8px;padding:10px 18px;background:#5865F2;color:#fff;border-radius:8px;font-size:14px;font-weight:500;text-decoration:none;transition:background .15s"
