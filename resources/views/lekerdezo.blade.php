...
@section('content')
<h1 style="color:var(--fg);font-size:24px;font-weight:700;margin-bottom:24px">Lekérdező</h1>
...
@if($query !== null)
    ...
    <div class="card">
        ...
        <div style="padding:12px 0;border-bottom:1px solid var(--border)">
            ...
            <div style="color:var(--fg-subtle);font-size:12px">{{ $report->author->in_game_name ?? $report->author->name }} · {{ $report->created_at->format('Y. m. d.') }}</div>
        </div>
        @endforeach
        <div style="margin-top:16px">{{ $reports->links() }}</div>
    </div>
    @else
    <div class="card"><p style="color:var(--fg-subtle);font-size:14px">Nincs találat: „{{ $query }}"</p></div>
    @endif
@endif
@endsection
